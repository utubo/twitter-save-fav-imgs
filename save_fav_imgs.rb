...
$TWITTER_TRY_COUNT = 2
# API実行
BOUNDARY = "--bboounnddaallyy"
def twitter_execute(uri, post_body = nil, limit = 10)
	0.upto($TWITTER_TRY_COUNT) { |try_count|
		begin
			res = ""
			Timeout.timeout(60) {
				if post_body then
					res = $ACCESS_TOKEN.post(uri, post_body)
				else
			...
			when Net::HTTPRedirection then
				if 0 < limit then
					...
					return twitter_execute(req, post_body, limit - 1)
				else
			...
			when Net::HTTPNotFound, Net::HTTPForbidden, Net::HTTPBadGateway then
				...
				Log.warn("unknown responce code => #{res.code} msg => #{res.message} body => #{res.body}")
			end
		rescue => e
			Log.error(e) if try_count == $TWITTER_TRY_COUNT
			Log.warn(e)
...
def get_api(api_url)
	return twitter_execute(api_url)
end
...
def add_json(src, since_id, max_id = nil)
	last_id = since_id
	first_id = max_id
	Log.warn("src #=> nil") if src == nil || src.empty?
	json_obj = JSON.parse(src)
	json_obj.map { |status|
		id = status['id_str'];
		...
		a = Time::parse(status['created_at'])
		created_at = a.localtime.strftime('%Y%m%d_%H%M%S')
		...
		media.map { |m|
			...
			url = video_info == nil ? m['media_url'] : video_info['variants'][0]['url']
			ext = url.sub(/.+\//, '').sub(/.+\./, '').sub(/[\?#;].*/, '')
		...
		#variants = video_info != nil ? media['variants'] : nil
		}
	}
...
for i in 1..roop_count do
	...
	url = $FAV_URL
	url = url + '&since_id='+ last_id.to_s if !last_id.to_s.empty? && roop_count == 1
	url = url + '&max_id='+ max_id.to_s if !max_id.to_s.empty? && roop_count != 1
	Log.info("execute-api #{url}");
	res = get_api(url);
	last_id, max_id = add_json(res, last_id, max_id);
end
if (roop_count == 1)
